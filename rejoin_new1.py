...
def organize_windows(packages):
    """
    Redimensiona todos os Roblox para um tamanho padrão.
    Nota: Em Android, o comando 'wm size' altera a resolução da tela inteira.
    Para redimensionar janelas individuais, usamos o modo 'freeform'.
    """
    width = 600
    height = 400
    log(f"📐 Padronizando todas as janelas para {width}x{height}...")
    
    for pkg in packages:
        # Força o app a entrar em modo freeform/janela se o dispositivo suportar
        # Isso é comum em emuladores e Androids com 'Opções do Desenvolvedor' ativas
        run_adb(f"am start --task-windowing-mode 5 -n {pkg}/{PROTO_ACTIVITY}")
        time.sleep(1)
...
def reconnect(package, vip_link):
    ...
    time.sleep(2)
    log("🌐 Abrindo VIP...")
    # Abre em modo freeform (janela) se possível
    run_adb(f"am start --task-windowing-mode 5 -n {package}/{PROTO_ACTIVITY} -a android.intent.action.VIEW -d '{vip_link}'")
    time.sleep(8)
...
def main():
    ...
    # Organiza as janelas logo no início
    organize_windows(packages)
