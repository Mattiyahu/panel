...
def get_screen_size():
    ...
    return 1080, 1920  # Padrao

def position_window(pkg, x, y, width, height):
    """
    Posiciona uma janela em coordenadas especificas usando am stack.
    Funciona em modo freeform/multi-window.
    """
    # Primeiro, obtem o task ID do pacote
    task_out = adb(f"am stack list | grep {pkg}")
    task_match = re.search(r'taskId=(\d+)', task_out)
    
    if task_match:
        task_id = task_match.group(1)
        # Move a janela para a posicao especificada
        adb(f"am stack positiontask {task_id} 1 {x} {y} {x+width} {y+height}")
    else:
        # Alternativa: usar input para redimensionar manualmente
        pass

def close_all_roblox():
    """Fecha todos os apps Roblox"""
    pkgs = get_packages()
    for pkg in pkgs:
        stop_app(pkg)
        time.sleep(0.3)
...
def open_all_in_grid(pkgs, vip_link, check_key_callback=None):
    """
    Abre todos os Roblox em grade, verificando key de cada um.
    
    Args:
        pkgs: Lista de pacotes
        vip_link: Link VIP para abrir
        check_key_callback: Funcao para verificar/processar key (opcional)
    """
    if not pkgs:
        return
    ...
    # Calcula grid baseado no numero de pacotes
    if num_pkgs == 1:
        cols, rows = 1, 1
    elif num_pkgs == 2:
        cols, rows = 2, 1
    ...
    for i, pkg in enumerate(pkgs):
        ...
        # Calcula posicao na grade
        col = i % cols
        row = i // cols
        x = col * win_w
        y = row * win_h
        
        # Abre o app com VIP link
        start_app_with_vip(pkg, vip_link)
        time.sleep(3)  # Espera carregar
        
        # Traz para foco para verificar key
        bring_to_focus(pkg)
        time.sleep(1)
        
        # Verifica se precisa de key
        if check_key_callback:
            xml = get_ui_xml()
            if detect_key_screen(xml):
                check_key_callback(pkg, xml)
                results.append((name, "KEY_PROCESSED"))
            else:
                results.append((name, "OK"))
        else:
            results.append((name, "OPENED"))
        
        # Tenta posicionar a janela (pode nao funcionar em todos os dispositivos)
        position_window(pkg, x, y, win_w, win_h)
...
class Monitor:
    ...
    def start(self):
        """Inicia o monitor com inicializacao limpa"""
        vip = CONFIG.get("vip_link", "")
        if not vip:
        ...
        # INICIALIZACAO LIMPA
        # ═══════════════════════════════════════════════════════════
        rprint("[bold yellow]\n=== INICIALIZACAO LIMPA ===[/bold yellow]")
        
        # Passo 1: Fecha todos os Roblox
        rprint("[red]Fechando todos os Roblox...[/red]")
        for pkg in pkgs:
            stop_app(pkg)
            name = pkg.split('.')[-1].upper()
            rprint(f"  [dim]X {name}[/dim]")
            time.sleep(0.3)
        time.sleep(1)
        
        # Passo 2: Abre cada um sequencialmente e verifica key
        rprint("[green]\nAbrindo e verificando cada instancia...[/green]")
        
        for i, pkg in enumerate(pkgs):
            name = pkg.split('.')[-1].upper()
            rprint(f"\n[bright_red]>>> {name} ({i+1}/{len(pkgs)})[/bright_red]")
            
            # Abre com VIP link
            rprint(f"  [dim]Abrindo com VIP...[/dim]")
            start_app_with_vip(pkg, vip)
            ...
            # Verifica se precisa de key
            if detect_key_screen(xml):
                rprint(f"  [yellow]KEY DETECTADA! Processando...[/yellow]")
                self.key_bypass.process(pkg, xml)
                rprint(f"  [green]Key processada![/green]")
            elif detect_home_screen(xml):
                rprint(f"  [yellow]Tela HOME detectada, reabrindo...[/yellow]")
                stop_app(pkg)
                time.sleep(1)
                start_app_with_vip(pkg, vip)
                time.sleep(3)
            elif detect_disconnected(xml):
                rprint(f"  [yellow]Desconectado, reabrindo...[/yellow]")
                stop_app(pkg)
                time.sleep(1)
                start_app_with_vip(pkg, vip)
                time.sleep(3)
            else:
                rprint(f"  [green]OK - Jogo carregando[/green]")
            
            time.sleep(1)
        
        rprint("\n[bold green]=== INICIALIZACAO COMPLETA ===[/bold green]")
        rprint("[dim]Iniciando monitoramento passivo em 3s...[/dim]")
        time.sleep(3)
        
        # ═══════════════════════════════════════════════════════════
        # INICIA MONITORAMENTO
        # ═══════════════════════════════════════════════════════════
        self.logs = []  # Limpa logs
        self.log(f"Inicializacao completa: {len(pkgs)} instancias")

        # Cria instancias e inicia workers de monitoramento passivo
        for pkg in pkgs:
            inst = Instance(pkg)
            inst.cooldown_until = time.time() + 60  # 1 min de cooldown inicial
        ...
        # Inicia worker de verificacao com foco
